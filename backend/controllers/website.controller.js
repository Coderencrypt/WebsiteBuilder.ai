...
// 👇 helper function to build a unique, URL-safe slug
        ...
        if(user.credits<50){
          return res.status(400).json({message:"you have not enough credits to generate a website"})
            ...
            const website = await Website.create({
              ...
              slug: generateSlug(prompt), // 👈 added
              conversation:[
                {
                  role:"user",
                  content:prompt
...
return res.status(201).json({
  ...
  remainingCredits:user.credits
      ...
      console.log("=======Backend Bug Found =====",error);
    ...
    try {
       const {prompt} = req.body
        ...
        RETURN RAW JSON ONLY:
        {
          ...
          CRITICAL: The updated HTML code string must be readable, well-indented, and structurally separated using literal newlines (\\n).
           Do NOT compress or stringify the HTML payload onto a single line.
